...
const DropComp: React.VFC<DropCompProps> = ({ metaDataList, rows, cols, value, setMetaDataList }) => {

    const [{ isOver }, drop] = useDrop({
        accept: "card",
        drop: ((item: MetaData) => { addDataToBoard(item) }),
        collect: ((monitor: any) => ({
    ...
    const addDataToBoard = (item: MetaData) => {
        if(metaDataList.some(a=>(a.cord[0]==colIndex &&a.cord[1]==rowIndex))){
            return 
        }
    ...
    return (
        <Grid item key={value} xs={colSize[cols]} ref={drop} justifyContent="center" >
            <Item sx={{ border: "1px solid black",borderRadius: "2px",boxShadow:"0"}} >
